const socket = io("http://54.226.26.94/message");
let msgType = "user";
let maxMessages = 30;
let currentuserId;
let currentGrpId;
...
//     let queryString;
//     if (msgType === "group") {
//         queryString = `http://54.226.26.94/message/msg-after-id/${lastMsgId}?groupId=${currentGrpId}`
//     } else {
//         queryString = `http://54.226.26.94/message/msg-after-id/${lastMsgId}?receiverId=${currentuserId}`
//     }
...
function handleSearchedUser(e) {
    ...
    axios.get(`http://54.226.26.94/user/by_name/${name.value}`, {
        headers: {
            Authorization: usertoken
        }
    }).then(res => {
...
function getAllFriendRequest(e) {
    ...
    axios.get(`http://54.226.26.94/user/friend_request`, {
        headers: {
            Authorization: usertoken
        }
    }).then(res => {
...
function handelFriendRequest(e) {
    ...
    if (status) {
        axios.put(`http://54.226.26.94/user/friend_request/handle/${contactId}`, { status }, {
            headers: {
                Authorization: usertoken
            }
        }).then(res => {
...
function handelSendFriendRequest(e) {
    ...
    // axios.post(`http://54.226.26.94/user/friend_request/send`, { contactUserId }, {
    //     headers: {
    //         Authorization: usertoken
    //     }
    // }).then(res => {
...
function handleSearchedMember(e) {
    ...
    axios.get(`http://54.226.26.94/group/admin/contact_name/${name.value}`, {
        headers: {
            Authorization: usertoken
        }
    }).then(res => {
...
function handleAddMemberInGroup(e) {
    ...
    if (confirm("do you want to add this user in group")) {
        axios.get(`http://54.226.26.94/group/admin/add_user?groupId=${currentGrpId}&userId=${id} `, {
            headers: {
                Authorization: usertoken
            },
...
function handleGroupModalForm(e) {
    ...
    axios.post("http://54.226.26.94/group/create", GroupDetails, {
        headers: {
            Authorization: usertoken
        }
    }).then(res => {
...
function fetchAllChatsNames() {
    axios.get("http://54.226.26.94/group/fetch_all", {
        headers: {
            Authorization: usertoken,
        },
        withCredentials: true
...
function handleShareGroup(e) {
    e.preventDefault();
    let link = `http://54.226.26.94/group/add_request/?groupId=${e.currentTarget.parentElement.cid}`;
...
function handleLeaveGroup(e) {
    ...
    if (confirm("do you want to leave the group!")) {
        axios.post("http://54.226.26.94/group/leave", { groupId: currentGrpId }, {
            headers: {
                Authorization: usertoken,
            }
        }).then(res => {
...
function handleGroupMember(e) {
    e.preventDefault();
    axios.get(`http://54.226.26.94/group/members/?groupId=${currentGrpId}`, {
        headers: {
            Authorization: usertoken,
        }
    }).then(res => {
...
function handelGroupAdminMethods(e) {
    ...
    if (e.target.className.includes('makeadmin')) {
        if (confirm(`do you want to Make ${e.target.textContent} admin!`)) {
            ...
            axios.put(`http://54.226.26.94/group/admin/make_admin`, { userId: memberId, groupId: currentGrpId }, {
                headers: {
                    Authorization: usertoken,
                }
            }).then(res => {
    ...
    if (e.target.parentNode.className.includes('removeadmin') && e.target.className.includes('removeadminofgroup')) {
        if (confirm(`do you want to remove this member from admin of this group!`)) {
            ...
            axios.put(`http://54.226.26.94/group/admin/remove_admin`, { userId: memberId, groupId: currentGrpId }, {
                headers: {
                    Authorization: usertoken,
                }
            }).then(res => {
    ...
    if (e.target.parentNode.className.includes('remove member') && e.target.className.includes('removeFromGroup')) {
        if (confirm("do you want to remove this member!")) {
            const memberId = e.target.parentNode.parentNode.id;
            axios.delete(`http://54.226.26.94/group/admin/member/${memberId}?groupId=${currentGrpId}`, {
                headers: {
                    Authorization: usertoken,
                }
            }).then(res => {
